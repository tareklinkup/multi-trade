...
class Page extends CI_Controller {
        ...
        if($query->num_rows() > 0){
           ...
           echo json_encode($exists);
            ...
            $data = array(
                ...
                "status"    => 'a',
                "is_sqr"    => $this->input->post('is_sqr', TRUE),
                "AddBy"     => $this->session->userdata("FullName"),
                "AddTime"   => date("Y-m-d H:i:s")
                );
        ...
        try{
            ...
            if($nameCount > 0){
                ...
                if (!empty($_FILES['image'])) {
                    ...
                    move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/branch/' .$image);
                }
                $image_name = $image;
    ...
    public function updateBranch()  {
        ...
        try{
            ...
            $image_check = $this->db->query("select * from tbl_brunch where brunch_id != ?", [$branch->branchId])->row();
           
            $image_name = '';
            if (!empty($_FILES['image'])) {
                if(file_exists('/uploads/branch/'.$image_check->image && $image_check->image != null )){
                    unlink("/uploads/branch/".$image_check->image);
                }
                $image = uniqid().'_'.$_FILES['image']['name'];
                move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/branch/' . $image);
                $image_name = $image;
            }else{
                $image_name = $image_check->image;
            }
            $newBranch = array(
                'Brunch_name' => $branch->name,
                'Brunch_title' => $branch->title,
